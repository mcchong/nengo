...
class Builder(object):
    ...
    def __init__(self):
        ...
        self.gain = {}  # XXX temporary, until better probes
        self.bias = {}  # XXX temporary, until better probes
        self.encoders = {}  # XXX temporary, until better probes
        self.probes = []
        self.operators = []
        self.probemap = {}
        self.sig_in = {}
    ...
    def build_ensemble(self, ens):
        ...
        self.gain[ens.neurons] = ens.neurons.gain
        self.bias[ens.neurons] = ens.neurons.bias
        if self.gain[ens.neurons] is None or self.bias[ens.neurons] is None:
            ...
            self.gain[ens.neurons], self.bias[ens.neurons] = (
        ...
        self.encoders[ens] = encoders

        if isinstance(ens.neurons, nengo.Direct):
            scaled_encoders = encoders
        else:
            scaled_encoders = encoders * (self.gain[ens.neurons]
    ...
    def build_probe(self, probe):
        ...
        # Set up probe
        probe.probe = SimulatorProbe(self.sig_in[probe], probe.sample_every)
    ...
    def build_connection(self, conn):
        ...
        if (isinstance(conn.pre, nengo.Ensemble)
            ...
            x = np.dot(conn.eval_points,
                       self.encoders[conn.pre].T / conn.pre.radius)
            activities = self.dt * conn.pre.neurons.rates(
                x, self.gain[conn.pre.neurons], self.bias[conn.pre.neurons])
            if conn.function is None:
        ...
        if isinstance(conn.post, nengo.nonlinearities.Neurons):
            transform *= self.gain[conn.post][:, np.newaxis]
    ...
    def build_neurons(self, neurons):
        ...
        bias_signal = Signal(self.bias[neurons], name=neurons.label + '.bias')
